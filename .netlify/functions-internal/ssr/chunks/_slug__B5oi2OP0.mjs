...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Base", $$Base, { "lang": "en", "title": postIndex?.data.title }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="section"> <div class="container"> <h1 class="mb-10 text-center font-normal lg:text-[56px]">${unescapeHTML(markdownify(postIndex?.data.title))}</h1> <div class="row"> ${currentPosts.map((post) => renderTemplate`<div class="col-12 mb-8 sm:col-6"> ${renderComponent($$result2, "Post", $$Post, { "post": post })} </div>`)} </div> ${renderComponent($$result2, "Pagination", $$Pagination, { "section": config.settings.blog_folder, "totalPages": totalPages, "currentPage": currentPage })} </div> </section> ` })}`;
